...
// Firewall A resource that controls incoming and outgoing network traffic to a Linode service. A Firewall can be assigned to multiple Linode services, and up to five active Firewalls can be assigned to a single Linode service. [Create a Firewall Device](/docs/api/networking/#firewall-create) to assign a Firewall to a Linode service. Currently, Firewalls can only be assigned to Linode instances. 
type Firewall struct {
	...
	// The status of this Firewall.    * When a Firewall is first created its status is `enabled`.   * Use the [Update Firewall](/docs/api/networking/#firewall-update) endpoint to set a Firewall's status to `enbaled` or `disabled`.   * Use the [Delete Firewall](/docs/api/networking/#firewall-delete) endpoint to delete a Firewall. 
