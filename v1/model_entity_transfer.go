...
type EntityTransfer struct {
	...
	// The status of the transfer request.  `accepted`: The transfer has been accepted by another user and is currently in progress. Transfers can take up to 3 hours to complete.  `cancelled`: The transfer has been cancelled by the sender.  `completed`: The transfer has completed successfully.  `failed`: The transfer has failed after initiation.  `pending`: The transfer is ready to be accepted.  `stale`: The transfer has exceeded its expiration date. It can no longer be accepted or cancelled. 
...
func (o *EntityTransfer) GetEntities() EntityTransferEntities {
	if o == nil || o.Entities == nil {
		var ret EntityTransferEntities
		return ret
	}
	return *o.Entities
}
...
// SetEntities gets a reference to the given EntityTransferEntities and assigns it to the Entities field.
